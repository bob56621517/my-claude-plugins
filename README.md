...
| 插件 | 安装命令 | 功能 |
|------|---------|------|
| feature-workflow | `/plugin install feature-workflow@my-claude-plugins` | Feature 开发规范化流程——计划→审批→执行→验证 |
...
- `feature-workflow` — Feature 开发规范化流程，计划→审批→执行→验证，按模块/API 维度逐步推进
