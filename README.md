...
| unified-search | `/plugin install unified-search@my-claude-plugins` | 统一搜索路由——自动选择最佳搜索后端（已捆绑 Context7/Wikidata/博查三个搜索 MCP，一次安装即用） |
...
- `unified-search` — 统一搜索路由，自动选择最佳搜索后端（捆绑 Context7/Wikidata/博查）
...
| 变量 | 用途 | 所属插件 |
|------|------|---------|
| `BOCHA_API_KEY` | 博查搜索 API Key | unified-search |
